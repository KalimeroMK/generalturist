...
@section('content')
    <form action="{{route('type.admin.store',['type'=>$typeId,'id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}"
          method="post">
        @csrf
        <div class="container-fluid">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    ...
                    @if($row->slug)
                        <p class="item-url-demo">{{__("Permalink")}}: {{ url($type->getRoutePrefix() ) }}/<a href="#"
                ...
                <div class="">
                    @if($row->slug)
                        <a class="btn btn-primary btn-sm" href="{{$row->getDetailUrl(request()->query('lang'))}}"
                           target="_blank">{{__("View :type",['type'=>$typeName])}}</a>
                    @endif
                </div>
            </div>
