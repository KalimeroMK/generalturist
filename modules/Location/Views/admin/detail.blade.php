@php use Modules\Location\Hook; @endphp
@php use Modules\Media\Helpers\FileHelper; @endphp
@php use App\Helpers\MapEngine; @endphp
@extends('admin.layouts.app')

@section('content')
    <form action="{{route('location.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}"
          method="post">
        @csrf
                ...
                <div class="">
                    ...
                    @if($row->slug)
                        <p class="item-url-demo">{{__("Permalink")}}
                            : {{ url( (request()->query('lang') ? request()->query('lang').'/' : '').config('location.location_route_prefix'))  }}
                            /<a href="#" class="open-edit-input" data-name="slug">{{$row->slug}}</a></p>
                    @endif
                </div>
                <div class="">
                    @if($row->slug)
                    ...
                    <div class="col-md-9">
                        <div class="panel">
                            <div class="panel-body">
                                ...
                                <div class="form-group-item">
                                            ...
                                            @if(count($translation->trip_ideas))
                                                @foreach($translation->trip_ideas as $key=>$trip_idea)
                                                    <div class="item" data-number="{{$key}}">
                                                        <div class="row">
                                                            <div class="col-md-2">
                                                                {!! FileHelper::fieldUpload('trip_ideas['.$key.'][image_id]',$trip_idea['image_id']) !!}
                                                            </div>
                                                            <div class="col-md-4">
                                                                <input type="text" name="trip_ideas[{{$key}}][title]"
                                                                       class="form-control"
                                                                       value="{{$trip_idea['title']}}"
                                                                       placeholder="{{__("Title:")}}">
                                                                <input type="text" name="trip_ideas[{{$key}}][link]"
                                                                       class="form-control"
                                                                       value="{{$trip_idea['link']}}"
                                                                       placeholder="{{__("Link:")}}">
                                                            </div>
                                                            <div class="col-md-5">
                                                                <textarea name="trip_ideas[{{$key}}][content]"
                                                                          class="form-control full-h"
                                                                          placeholder="...">{{$trip_idea['content']}}</textarea>
                                                            </div>
                                                            <div class="col-md-1">
                                    ...
                                    <div class="g-more hide">
                                        <div class="item" data-number="__number__">
                                            <div class="row">
                                                <div class="col-md-2">
                                                    {!! FileHelper::fieldUpload('trip_ideas[__number__][image_id]','','__name__') !!}
                                                </div>
                                                <div class="col-md-4">
                                                    <input type="text" __name__="trip_ideas[__number__][title]"
                                                           class="form-control" placeholder="{{__("Title:")}}">
                                                    <input type="text" __name__="trip_ideas[__number__][link]"
                                                           class="form-control" placeholder="{{__("Link:")}}">
                                                </div>
                                                <div class="col-md-5">
                                                    <textarea __name__="trip_ideas[__number__][content]"
                                                              class="form-control full-h" placeholder="..."></textarea>
                                                </div>
                                                <div class="col-md-1">
                                                    <span class="btn btn-danger btn-sm btn-remove-item"><i
                                                                class="fa fa-trash"></i></span>
                                ...
                                <?php
                                do_action(Hook::FORM_AFTER_TRIP_IDEA, $row) ?>
                            </div>
                        </div>
