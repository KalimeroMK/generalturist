...
@section('content')
    <div class="container">
        <div class="d-flex justify-content-between mb20">
            ...
            <a class="btn btn-primary" href="{{url('admin/module/language/translations/loadStrings')}}"><i
                        class="icon ion-ios-search"></i> {{__("Find Translations")}}</a>
        </div>
                                ...
                                @if(count($languages) > 0)
                                    @foreach($languages as $language)
                                        <tr>
                                            ...
                                            <td>{{$total_text ? number_format((float) $language->translated_number / $total_text * 100,2) : 0 }}
                                                %
                                            </td>
                                            ...
                                            <td>
                                                <a href="{{url('admin/module/language/translations/detail/'.$language->id)}}"
                                                   class="btn btn-sm btn-primary"><i
                                                            class="fa fa-edit"></i> {{__("Translate")}}</a>
                                                <a href="{{url('admin/module/language/translations/build/'.$language->id)}}"
                                                   class="btn btn-sm btn-info"><i
                                                            class="fa fa-cubes"></i> {{__("Build")}}</a>
                                                ...
                                                <div class="dropdown-menu">
                                                    <a class="dropdown-item"
                                                       href="{{url('admin/module/language/translations/loadTranslateJson?locale='.$language->locale)}}">{{__("Load Translate Default")}}</a>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
