...
class AvailabilityController extends FrontendController
{
    ...
    public function loadDates(Request $request)
    {
        ...
        $bookings = $model_car->getBookingInRanges($car->id, $car->type, $request->query('start'),
            $request->query('end'));
        if (!empty($bookings)) {
                ...
                foreach ($period as $dt) {
                    ...
                    if (isset($allDates[date('Y-m-d', $i)])) {
                        ...
                        if ($total_number_booking >= $max_number) {
                            ...
                            $allDates[date('Y-m-d', $i)]['title'] = format_money_main($allDates[date('Y-m-d',
                                    $i)]['price']).' x '.($max_number - $total_number_booking);
                        }
                    }
                }
            }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($period as $dt) {
            $date = $this->carDateClass::where('start_date', $dt->format('Y-m-d'))->where('target_id',
                $target_id)->first();
