...
class Car extends Bookable
{
    ...
    static public function getSeoMetaForPageList()
    {
        ...
        if (!empty($title = setting_item_with_lang("car_page_list_seo_title", false))) {
            ...
            if (!empty($title = setting_item_with_lang("car_page_search_title"))) {
                $meta['seo_title'] = $title;
            }
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("car_page_list_seo_image"))) {
            ...
            if (!empty($title = setting_item("car_page_search_banner"))) {
                $meta['seo_image'] = $title;
            }
        }
    ...
    public function isAvailableInRanges($start_date, $end_date, $number = 1)
    {
        ...
        if (!empty($datesData)) {
            foreach ($datesData as $date) {
                ...
                if (!$date->active or !$date->number or !$date->price) {
                    return false;
                }
    ...
    public function getReviewList()
    {
        ...
        ])->where('object_id', $this->id)->where('object_model', 'car')->where("status", "approved")->orderBy("id",
            "desc")->with('author')->paginate(setting_item('car_review_number_per_page', 5));
    }
    ...
    public function getScoreReview()
    {
        ...
        $list_score = Cache::rememberForever('review_'.$this->type.'_'.$car_id, function () use ($car_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id',
                $car_id)->where('object_model', "car")->where("status", "approved")->first();
