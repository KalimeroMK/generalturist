...
class PayoutRequestNotificationListener
{
    ...
    protected function sendEmailCreate($event)
    {
        ...
        Mail::to($event->user->email)->send(new PayoutRequestCreatedEmail($event->user, $event->payout_request,
            'vendor'));
        ...
        if (!empty(setting_item('admin_email'))) {
            Mail::to(setting_item('admin_email'))->send(new PayoutRequestCreatedEmail($event->user,
                $event->payout_request, 'admin'));
        }
    }
    ...
    protected function sendEmailUpdate($event)
    {
        ...
        Mail::to($event->user->email)->send(new PayoutRequestChangedEmail($event->user, $event->payout_request,
            'vendor'));
        ...
        if (!empty(setting_item('admin_email'))) {
            Mail::to(setting_item('admin_email'))->send(new PayoutRequestChangedEmail($event->user,
                $event->payout_request, 'admin'));
        }
    }
    ...
    protected function sendEmailReject($event)
    {
        ...
        Mail::to($event->user->email)->send(new PayoutRequestRejectedEmail($event->user, $event->payout_request,
            'vendor'));
        ...
        if (!empty(setting_item('admin_email'))) {
            Mail::to(setting_item('admin_email'))->send(new PayoutRequestRejectedEmail($event->user,
                $event->payout_request, 'admin'));
        }
    }
    ...
    protected function sendEmailDelete($event)
    {
        ...
        Mail::to($event->user->email)->send(new PayoutRequestDeletedEmail($event->user, $event->payout_request,
            'vendor'));
        ...
        if (!empty(setting_item('admin_email'))) {
            Mail::to(setting_item('admin_email'))->send(new PayoutRequestDeletedEmail($event->user,
                $event->payout_request, 'admin'));
        }
    }
