...
@if($available_payout_amount)
    ...
    <br>
    <div class="">
        <a href="#vendor_create_request" data-toggle="modal" class="btn btn-success btn-sm"><i
                    class="fa fa-plus"></i> {{__("Create request")}}</a>
    </div>
@else
        ...
        <div class="modal-content">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" class="form-control" readonly
                                   value="{{format_money($available_payout_amount)}}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label">{{__("Amount")}} <span
                                    class="text-danger">*</span></label>
                        <div class="col-sm-9">
                            <input type="number" required max="{{$available_payout_amount}}" class="form-control"
                                   name="amount" value="{{$available_payout_amount}}">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-sm-3 col-form-label">{{__("Method")}} <span
                                    class="text-danger">*</span></label>
                        <div class="col-sm-9">
                            <select required class="form-control" name="payout_method">
                                <option value="">{{__('-- Please select --')}}</option>
                                @foreach($currentUser->available_payout_methods as $id=>$method)
                                    <option value="{{$id}}">{{$method->name}} @if(!empty($method->min))
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-success "
                        onclick="vendorPayout.sendRequest(this)">{{__('Send request')}}
