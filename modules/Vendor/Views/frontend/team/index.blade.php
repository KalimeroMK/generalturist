    ...
    <form method="post" action="{{route('vendor.team.add')}}">
        ...
        <div class="row">
            <div class="col-md-3">
                ...
                <input type="email" value="{{old('email')}}" name="email" required class="form-control"
                       placeholder="{{__("Email address")}}" aria-label="{{__("Email address")}}"
                       aria-describedby="button-addon2">
            </div>
            <div class="col-md-3">
                <label class="font-weight-bold">{{__("Permissions")}}</label>
                @foreach(get_bookable_services() as $service_id=>$service)
                    <div><label><input @if(in_array($service_id,old('permissions',[]))) checked @endif type="checkbox"
                                       name="permissions[]" value="{{$service_id}}">{{$service::getModelName()}}</label>
                    </div>
                @endforeach
            </div>
        </div>
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                <a class="dropdown-item"
                                   href="{{route('vendor.team.edit',['vendorTeam'=>$vendorTeam])}}">{{__("Edit")}}</a>
                                ...
                                @endif
                                <a class="dropdown-item"
                                   href="{{URL::signedRoute('vendor.team.delete',['vendorTeam'=>$vendorTeam->id])}}">{{__("Delete")}}</a>
                            </div>
                        </div>
                    </td>
                </tr>
