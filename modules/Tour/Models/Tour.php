...
class Tour extends Bookable
{
    ...
    static public function getSeoMetaForPageList()
    {
        ...
        if (!empty($title = setting_item_with_lang("tour_page_list_seo_title", false))) {
            ...
            if (!empty($title = setting_item_with_lang("tour_page_search_title"))) {
                $meta['seo_title'] = $title;
            }
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("tour_page_list_seo_image"))) {
            ...
            if (!empty($title = setting_item("tour_page_search_banner"))) {
                $meta['seo_image'] = $title;
            }
        }
    ...
    static public function getFiltersSearch()
    {
        ...
        $category = TourCategory::selectRaw("id,name,slug")->where('status',
            'publish')->with(['translation'])->get()->toTree();
        return [
            [
    ...
    public function getReviewDataAttribute()
    {
        ...
        $dataTotalReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id',
            $this->id)->where('object_model', "tour")->where("status", "approved")->first();
    ...
    public function getScoreReview()
    {
        ...
        $list_score = Cache::rememberForever('review_'.$this->type.'_'.$tour_id, function () use ($tour_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id',
                $tour_id)->where('object_model', "tour")->where("status", "approved")->first();
            return [
    ...
    {
        $tourDate = $this->tourDateClass::where('target_id', $this->id)->where('start_date',
            $start_date)->where('active', 1)->first();
        $totalGuests = $this->bookingClass::where('object_id', $this->id)->where('start_date',
            $start_date)->whereNotIn('status', $this->bookingClass::$notAcceptedStatus)->sum('total_guests');
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if (!empty($location)) {
            $number = parent::join('bravo_locations', function ($join) use ($location) {
                $join->on('bravo_locations.id', '=', 'bravo_tours.location_id')->where('bravo_locations._lft', '>=',
