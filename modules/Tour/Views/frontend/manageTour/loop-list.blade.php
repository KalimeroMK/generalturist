    ...
    <div class="row">
        ...
        <div class="col-md-9">
            ...
            <div class="control-action">
                @if(!empty($recovery))
                    <a href="{{ route("tour.vendor.restore",[$row->id]) }}" class="btn btn-recovery btn-primary"
                       data-confirm="{{__('"Do you want to recovery?"')}}">{{__("Recovery")}}</a>
                    @if(Auth::user()->hasPermissionTo('tour_delete'))
                        <a href="{{ route("tour.vendor.delete",['id'=>$row->id,'permanently_delete'=>1]) }}"
                           class="btn btn-danger"
                           data-confirm="<?php echo e(__("Do you want to permanently delete?")); ?>">{{__("Del")}}</a>
                    @endif
                @else
                    <a href="{{route('tour.vendor.clone',[$row->id])}}" target="_blank"
                    ...
                    @endif
                    @if(Auth::user()->hasPermissionTo('tour_delete'))
                        <a href="{{ route("tour.vendor.delete",[$row->id]) }}" class="btn btn-danger"
                           data-confirm="<?php echo e(__("Do you want to delete?")); ?>">{{__("Del")}}</a>
                    @endif
                    @if($row->status == 'publish')
                        <a href="{{ route("tour.vendor.bulk_edit",[$row->id,'action' => "make-hide"]) }}"
                           class="btn btn-secondary">{{__("Make hide")}}</a>
                    @endif
                    @if($row->status == 'draft')
                        <a href="{{ route("tour.vendor.bulk_edit",[$row->id,'action' => "make-publish"]) }}"
