<div class="panel">
    ...
    <div class="panel-body">
        ...
        <div class="form-group">
            <label>
                <input type="checkbox" name="enable_fixed_date" @if(!empty($row->enable_fixed_date)) checked
                       @endif value="1"> {{__('Enable Fixed Date')}}
            </label>
        </div>
        <?php
        $old = $row->meta->open_hours ?? []; ?>
        <div class="row" data-condition="enable_fixed_date:is(1)">
            <div class="col-lg-3">
                <div class="form-group">
                    <label for="">{{__("Start Date")}}</label>
                    <input type="text" name="start_date" id=" start_date" class="form-control has-datepicker"
                           value="{{ old('start_date',!empty($row->start_date)?$row->start_date->format("Y-m-d"):"")}}">
            ...
            <div class="col-lg-3">
                <div class="form-group">
                    ...
                    <input type="text" name="end_date" id=" end_date" class="form-control has-datepicker"
                           value="{{ old('end_date',!empty($row->end_date)?$row->end_date->format("Y-m-d"):"")}}">
                </div>
            </div>
            <div class="col-lg-3">
                <div class="form-group">
                    <label for="">{{__("Last Booking Date")}}</label>
                    <input type="text" name="last_booking_date" id=" last_booking_date"
                           class="form-control has-datepicker"
                           value="{{ old('last_booking_date',!empty($row->last_booking_date)?$row->last_booking_date->format("Y-m-d"):"")}}">
                </div>
            </div>
        </div>
