        ...
        <div class="row">
            <div class="col-md-4 mb40">
                <div class="panel">
                    ...
                    <div class="panel-body">
                        <form action="{{route('tour.admin.attribute.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}"
                              method="post">
                            @csrf
            ...
            <div class="col-md-8">
                <div class="filter-div d-flex justify-content-between ">
                    <div class="col-left">
                        @if(!empty($rows))
                            <form method="post" action="{{route('tour.admin.attribute.editAttrBulk')}}"
                    ...
                    <div class="col-left">
                        <form method="get" action="{{route('tour.admin.attribute.index')}} "
                                ...
                                @if(count($rows) > 0)
                                    @foreach($rows as $row)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{route('tour.admin.attribute.edit', ['id' => $row->id]) }}"
                                                   class="btn btn-primary btn-sm"><i
                                                            class="fa fa-edit"></i> {{__('Edit')}}
                                                </a>
                                                <a href="{{route('tour.admin.attribute.term.index', ['attr_id' => $row->id]) }}"
