...
class TourController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($list)) {
            foreach ($list as $row) {
                $markers[] = [
                    ...
                    "infobox" => view('Tour::frontend.layouts.search.loop-grid',
                        ['row' => $row, 'disable_lazyload' => 1, 'wrap_class' => 'infobox-item'])->render(),
                    'marker' => get_file_url(setting_item("tour_icon_marker_map"),
        ...
        $data = [
            'rows' => $list,
            'tour_category' => $this->tourCategoryClass::where('status',
                'publish')->with(['translation'])->get()->toTree(),
            'tour_location' => $this->locationClass::where('status',
                'publish')->with(['translation'])->limit($limit_location)->get()->toTree(),
        ...
        $data['attributes'] = $this->attributesClass::where('service', 'tour')->orderBy("position",
            "desc")->with(['terms', 'translation'])->get();
        if ($layout == "map") {
    ...
    public function detail(Request $request, $slug)
    {
        ...
        if (!empty($location_id)) {
            $tour_related = $this->tourClass::where('location_id', $location_id)->where("status",
