...
@section('content')
    ...
    <form action="{{route('page.admin.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}"
          method="post">
        @csrf
        <div class="container">
            <div class="d-flex justify-content-between mb20">
                <div class="">
                    ...
                    @if($row->slug)
                        <p class="item-url-demo">{{ __('Permalink: ')}} {{ url((request()->query('lang') ? request()->query('lang').'/' : ''). config('page.page_route_prefix') )}}
                ...
                <div class="">
                    @if($row->id)
                        <a class="btn btn-primary btn-sm" href="{{route('page.admin.builder',['id'=>$row->id])}}"><i
                                    class="fa fa-paint-brush"></i> {{ __('Template Builder')}}</a>
                    @endif
                    @if($row->slug)
                        <a class="btn btn-primary btn-sm" href="{{$row->getDetailUrl(request()->query('lang'))}}"
                           target="_blank">{{ __('View page')}}</a>
                    @endif
                </div>
            </div>
                            ...
                            <div class="panel-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" value="{!! clean($translation->title) !!}"
                                           placeholder="Page title" name="title" class="form-control">
                                </div>
                                <div class="form-group">
                        ...
                        @if(is_default_lang())
                            ...
                            <div class="panel">
                                <div class="panel-body">
                                    ...
                                    <div class="form-group">
                                        {!! FileHelper::fieldUpload('custom_logo',$row->custom_logo) !!}
                                    </div>
                                </div>
                            </div>
                            <div class="panel">
