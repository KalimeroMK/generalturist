...
@section('content')
    <div class="container-fluid">
        ...
        <div class="filter-div d-flex justify-content-end ">
            <div class="col-left">
                @if(!empty($fields))
                    <form method="post" action="{{route('user.admin.role.bulkEdit')}}"
        ...
        <div class="row">
            <div class="col-md-4">
                <form method="post" action="{{route('user.admin.role.verifyFieldsStore')}}" class="needs-validation"
                      novalidate>
                    @csrf
                    <div class="panel">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    @foreach($fields as $id=>$row)
                                        <tr>
                                            <td><input type="checkbox" name="ids[]" value="{{$id}}" class="check-item">
                                            </td>
                                            <td>{{$id}}</td>
                                            ...
                                            <td>@php
                                                    if(!empty($row['roles'])){
                                                        $roles = Role::query()->whereIn('id',$row['roles'])->get();
                                                        if(!empty($roles))
                                                        {
                                            ...
                                            <th><a href="{{route('user.admin.role.verifyFieldsEdit',['id'=>$id])}}"
                                                   class="btn btn-primary btn-sm"> <i
                                                            class="fa fa-edit"></i> {{__('Edit')}}</a></th>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
