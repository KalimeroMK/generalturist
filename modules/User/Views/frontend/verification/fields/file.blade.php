        ...
        <div class="col-md-{{$value_col_size ?? 4}} btn-upload-private-wrap">
            <div class="private-file-lists mb-2">
                ...
                @if(!empty($old))
                    ...
                    <a target="_blank" href="{{route('media.private.view',['path'=>$old['path'] ?? '','v'=>uniqid()])}}"
                       class="file-item">{{$old['name']}} <i class="fa fa-download"></i></a>
                @endif
            </div>
            @if(empty($only_show_data))
                <span class="btn btn-primary btn-sm "><i class="fa fa-upload"></i> {{__('Select File')}}
                    <input class="btn-upload-private-file" data-name="verify_data_{{$field['id']}}" data-multiple=""
                           type="file">
                </span>
            @else
                @if(empty($field['data']))
