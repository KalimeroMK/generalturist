    ...
    <div class="row ">
        ...
        <div class="col-md-{{$value_col_size ?? 4}}">
            @if(empty($only_show_data))
                <input type="text" class="form-control mb-1" id="phone_verify_field" name="verify_data_{{$field['id']}}"
                       value="{{$field['data']}}">
                @if(empty($field['is_verified']))
                    <span class="btn btn-sm btn-primary"
                          onclick="sendCodeVerifyPhone('verify_data_{{$field["id"]}}','{{$fieldName}}')">{{__('Verify Phone')}}</span>
                @else
