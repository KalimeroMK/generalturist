...
class UserController extends AdminController
{
    ...
    public function getForSelect2(Request $request)
    {
        ...
        if ($q) {
            $query->where(function ($query) use ($q) {
                $query->where('first_name', 'like', '%'.$q.'%')->orWhere('last_name', 'like',
                    '%'.$q.'%')->orWhere('email', 'like', '%'.$q.'%')->orWhere('id', $q)->orWhere('phone', 'like',
                    '%'.$q.'%');
            });
        }
    ...
    public function userUpgradeRequest(Request $request)
    {
        ...
        $data = [
            'rows' => $listUser->whereHas('user')->with(['user', 'role', 'approvedBy'])->orderBy('id',
                'desc')->paginate(20),
            'roles' => Role::all(),
                ...
                foreach ($ids as $id) {
                    ...
                    if (!empty($vendorRequest)) {
                        $vendorRequest->update([
                            'status' => $action, 'approved_time' => now(), 'approved_by' => Auth::id()
                        ]);
