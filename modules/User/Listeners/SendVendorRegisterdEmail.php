...
class SendVendorRegisterdEmail
{
    ...
    public function handle(NewVendorRegistered $event)
    {
        ...
        if (!empty(setting_item('enable_mail_vendor_registered'))) {
            $body = $this->replaceContentEmail($event,
                setting_item_with_lang('vendor_content_email_registered', app()->getLocale()));
        ...
        if (!empty(setting_item('admin_email') and !empty(setting_item('admin_enable_mail_vendor_registered'))) and setting_item_with_lang('admin_content_email_vendor_registered',
                app()->getLocale())) {
            $body = $this->replaceContentEmail($event,
            ...
            foreach (self::CODE as $item => $value) {
                if ($item == "link_approved") {
                    $content = str_replace($value,
                        "<a href='".route('user.admin.upgrade')."'>".route('user.admin.upgrade')."</a>", $content);
