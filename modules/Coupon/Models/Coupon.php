...
class Coupon extends BaseModel
{
    ...
    public function applyCouponValidate($booking, $action)
    {
        ...
        if (!empty($min_total = $this->min_total) and $booking->total_before_discount < $min_total) {
            return [
                'status' => 0,
                'message' => __("The order has not reached the minimum value of :amount to apply the coupon code!",
                    ['amount' => format_money($min_total)])
            ];
        }
        if (!empty($max_total = $this->max_total) and $booking->total_before_discount > $max_total) {
            return [
                'status' => 0,
                'message' => __("This order has exceeded the maximum value of :amount to apply coupon code! ",
                    ['amount' => format_money($max_total)])
            ];
        }
        ...
        if (!empty($quantity_limit = $this->quantity_limit)) {
            $count = CouponBookings::where('coupon_code', $this->code)->whereNotIn('booking_status',
                ['draft', 'unpaid', 'cancelled'])->count();
        ...
        if (!empty($limit_per_user = $this->limit_per_user)) {
            ...
            $count = CouponBookings::where('coupon_code', $this->code)->where("create_user",
                $user_id)->whereNotIn('booking_status', ['draft', 'unpaid', 'cancelled'])->count();
