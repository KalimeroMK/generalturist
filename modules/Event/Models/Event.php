    ...
    class Event extends Bookable
    {
        ...
        static public function getSeoMetaForPageList()
        {
            ...
            if (!empty($title = setting_item_with_lang("event_page_list_seo_title", false))) {
                ...
                if (!empty($title = setting_item_with_lang("event_page_search_title"))) {
                    $meta['seo_title'] = $title;
                }
            }
            $meta['seo_image'] = null;
            if (!empty($title = setting_item("event_page_list_seo_image"))) {
                ...
                if (!empty($title = setting_item("event_page_search_banner"))) {
                    $meta['seo_image'] = $title;
                }
            }
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
                    ...
                    foreach ($ticketsAvailableBook as $k => $ticketBook) {
                        if (isset($ticket_types_input[$k]) and $ticket_types_input[$k]['number']) {
                            ...
                            if ($ticketBook["number"] < $currentNumberUserBook) {
                                ...
                                return $this->sendError(__("There are :numberTicket :titleTicket available for your selected date",
                                    ["numberTicket" => $ticketBook["number"], "titleTicket" => $title]));
                            }
                        }
                    }
                }
        ...
        public function getDataAvailableBooking($start_date)
        {
            ...
            $dataBooking = $this->bookingClass::select("id")->where('object_id', $this->id)->where('start_date',
                $start_date)->whereNotIn('status', $this->bookingClass::$notAcceptedStatus)->get();
        ...
        public function getReviewList()
        {
            return $this->reviewClass::select([
                ...
                'vendor_id',
                'create_user',
            ])->where('object_id', $this->id)->where('object_model', 'event')->where("status", "approved")->orderBy("id",
                "desc")->with('author')->paginate(setting_item('event_review_number_per_page', 5));
        }
        ...
        public function getScoreReview()
        {
            ...
            $list_score = Cache::rememberForever('review_'.$this->type.'_'.$event_id, function () use ($event_id) {
                $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id',
                    $event_id)->where('object_model', "event")->where("status", "approved")->first();
