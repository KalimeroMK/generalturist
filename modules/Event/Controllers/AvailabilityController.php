...
class AvailabilityController extends FrontendController
{
    ...
    public function loadDates(Request $request)
    {
        ...
        $bookings = $this->bookingClass::getAllBookingInRanges($event->id, $event->type, $request->query('start'),
            $request->query('end'));
        if (!empty($bookings)) {
                        ...
                        if ($event->getBookingType() == "time_slot") {
                            ...
                            foreach ($timeSlots as $item) {
                                ...
                                if (in_array($value, $allDates[$date]['booking_time_slots'])) {
                                    $allDates[$date]['booking_time_slots'] = array_diff($allDates[$date]['booking_time_slots'],
                                        [$value]);
                                }
                            }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($period as $dt) {
            $date = $this->eventDateClass::where('start_date', $dt->format('Y-m-d'))->where('target_id',
                $target_id)->first();
            if (empty($date)) {
