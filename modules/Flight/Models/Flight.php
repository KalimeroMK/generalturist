...
class Flight extends Bookable
{
    ...
    public static function searchCustom(Request $request)
    {
        ...
        if (is_array($terms) && !empty($terms)) {
            ...
            if (!empty($terms)) {
                $model_Flight->join('bravo_flight_term as tt', 'tt.target_id', "bravo_flight.id")->whereIn('tt.term_id',
                    $terms);
            }
        }
        ...
        if (!empty($lat = $request->query('map_lat')) and !empty($lgn = $request->query('map_lgn'))) {
            $model_Flight->orderByRaw("POW((bravo_flight.map_lng-?),2) + POW((bravo_flight.map_lat-?),2)",
                [$lgn, $lat]);
        }
