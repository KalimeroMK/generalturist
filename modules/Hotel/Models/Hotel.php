    ...
    class Hotel extends Bookable
    {
        ...
        static public function getSeoMetaForPageList()
        {
            ...
            if (!empty($title = setting_item_with_lang("hotel_page_list_seo_title", false))) {
                ...
                if (!empty($title = setting_item_with_lang("hotel_page_search_title"))) {
                    $meta['seo_title'] = $title;
                }
            }
            $meta['seo_image'] = null;
            if (!empty($title = setting_item("hotel_page_list_seo_image"))) {
                ...
                if (!empty($title = setting_item("hotel_page_search_banner"))) {
                    $meta['seo_image'] = $title;
                }
            }
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
        ...
        public function termsByAttributeInListingPage()
        {
            ...
            return $this->hasManyThrough($this->termClass, $this->hotelTermClass, 'target_id', 'id', 'id',
                'term_id')->where('bravo_terms.attr_id', $attribute)->with(['translations']);
        }
        ...
        public function addToCartValidate(Request $request)
        {
            ...
            foreach ($selected_rooms as $room) {
                ...
                if (!empty($rooms_by_id[$room['id']]->min_day_stays) and $numberDays < $rooms_by_id[$room['id']]->min_day_stays) {
                    return $this->sendError(__("The :name need to select at least :number days",
                        ['name' => $rooms_by_id[$room['id']]->title, 'number' => $rooms_by_id[$room['id']]->min_day_stays]));
                }
            }
            ...
            foreach ($rooms as $room) {
                if ($room->isAvailableAt($filters)) {
                    ...
                    $res[] = [
                        ...
                        'price_html'      => format_money($room->tmp_price).'<span class="unit">/'.($room->tmp_nights ? __(':count nights',
                                ['count' => $room->tmp_nights]) : __(":count night", ['count' => $room->tmp_nights])).'</span>',
                        'price_text'      => format_money($room->tmp_price).'/'.($room->tmp_nights ? __(':count nights',
                                ['count' => $room->tmp_nights]) : __(":count night", ['count' => $room->tmp_nights])),
                        'terms'           => $terms,
        ...
        public function check_allow_review_after_making_completed_booking()
        {
            ...
            if (!empty($options)) {
                ...
                $booking = $this->bookingClass::select("status")->where("object_id", $this->id)->where("object_model",
                    $this->type)->where("customer_id", Auth::id())->orderBy("id", "desc")->first();
        ...
        {
            $rooms = HotelRoomBooking::getByBookingId($booking->id);
        ...
        public function getReviewList()
        {
            return $this->reviewClass::select([
                ...
                'vendor_id',
                'create_user',
            ])->where('object_id', $this->id)->where('object_model', 'hotel')->where("status", "approved")->orderBy("id",
                "desc")->with('author')->paginate(setting_item('hotel_review_number_per_page', 5));
        }
        ...
        public function getScoreReview()
        {
            ...
            $list_score = Cache::rememberForever('review_'.$this->type.'_'.$hotel_id, function () use ($hotel_id) {
                $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id',
                    $hotel_id)->where('object_model', "hotel")->where("status", "approved")->first();
