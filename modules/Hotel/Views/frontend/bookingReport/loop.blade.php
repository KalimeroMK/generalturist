<tr>
    ...
    <td class="a-hidden">
        ...
        @if(!empty($rooms))
            @foreach($rooms as $room)
                <div class="label">{{$room->room->title}} * {{$room->number}}
                    = {{format_money_main($room->price * $room->number)}} </div>
            @endforeach
        @endif
    </td>
    ...
    <td width="2%">
        ...
        @if(!empty(setting_item("hotel_allow_vendor_can_change_paid_amount")))
            <a class="btn btn-xs btn-info btn-info-booking mt-1" data-toggle="modal"
