...
class Space extends Bookable
{
    ...
    static public function getSeoMetaForPageList()
    {
        ...
        if (!empty($title = setting_item_with_lang("space_page_list_seo_title", false))) {
            ...
            if (!empty($title = setting_item_with_lang("space_page_search_title"))) {
                $meta['seo_title'] = $title;
            }
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("space_page_list_seo_image"))) {
            ...
            if (!empty($title = setting_item("space_page_search_banner"))) {
                $meta['seo_image'] = $title;
            }
        }
    ...
    public static function getMinMaxPrice()
    {
        $model = parent::selectRaw('MIN( CASE WHEN sale_price > 0 THEN sale_price ELSE ( price ) END ) AS min_price ,
                                    MAX( CASE WHEN sale_price > 0 THEN sale_price ELSE ( price ) END ) AS max_price ')->where("status",
            "publish")->first();
    ...
    public function addToCart(Request $request)
    {
        ...
        if ($this->getBookingType() == 'by_night') {
            ...
            $duration_in_hour = max(1,
                ceil(($end_dateNight->getTimestamp() - $start_date->getTimestamp()) / HOUR_IN_SECONDS));
        } else {
            $duration_in_hour = max(1,
                ceil(($end_date->getTimestamp() - $start_date->getTimestamp()) / HOUR_IN_SECONDS) + 24);
        }
    ...
    public function addToCartValidate(Request $request)
    {
        ...
        if ($this->getBookingType() != 'by_night') {
            ...
            if (!empty($this->min_day_stays) and $numberDays < $this->min_day_stays) {
                return $this->sendError(__("You must to book a minimum of :number days",
                    ['number' => $this->min_day_stays]));
            }
        } else {
            $numberDays = (abs(strtotime($end_date) - strtotime($start_date)) / 86400);
            if (!empty($this->min_day_stays) and $numberDays < $this->min_day_stays) {
                return $this->sendError(__("You must to book a minimum of :number nights",
                    ['number' => $this->min_day_stays]));
            }
        }
    ...
    public function search($request)
    {
        ...
        if (is_array($terms) and !empty($terms = array_filter($terms))) {
            $model_space->join('bravo_space_term as tt', 'tt.target_id', "bravo_spaces.id")->whereIn('tt.term_id',
                $terms);
        }
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
    ...
    public function getReviewList()
    {
        ...
        ])->where('object_id', $this->id)->where('object_model', 'space')->where("status", "approved")->orderBy("id",
            "desc")->with('author')->paginate(setting_item('space_review_number_per_page', 5));
    }
    ...
    public function getScoreReview()
    {
        ...
        $list_score = Cache::rememberForever('review_'.$this->type.'_'.$space_id, function () use ($space_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id',
                $space_id)->where('object_model', "space")->where("status", "approved")->first();
