...
class AvailabilityController extends FrontendController
{
    ...
    public function loadDates(Request $request)
    {
        ...
        $bookings = $this->bookingClass::getBookingInRanges($space->id, $space->type, $request->query('start'),
            $request->query('end'));
        if (!empty($bookings)) {
...
//        {
        ...
        foreach ($period as $dt) {
            $date = $this->spaceDateClass::where('start_date', $dt->format('Y-m-d'))->where('target_id',
                $target_id)->first();
