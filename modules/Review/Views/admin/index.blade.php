...
@php use Modules\Review\Models\Review; @endphp
...
@section('content')
    <div class="container-fluid">
        ...
        <div class="filter-div d-flex justify-content-between ">
            <div class="col-left">
                @if(!empty($rows))
                    <form method="post" action="{{route('review.admin.bulkEdit')}}"
            ...
            <div class="col-left">
                <form method="post" action="{{route('review.admin.index')}} "
                    ...
                    @if(!empty($rows))
                            ...
                            ?>
                    @endif
                    <input type="text" name="s" value="{{ Request()->s }}" placeholder="{{__('Search by title')}}"
                           class="form-control">
                                                ...
                                                <div class="review_list_photos d-flex mt-3">
                                                    @foreach($listImages as $oneImages)
                                                        ...
                                                        <div class="review_upload_item"
                                                             style="background-image: url({{@$imagesData['download']}}); background-repeat: no-repeat; background-size: cover; background-position: center; height: 100px;width: 100px;margin-right: 10px;"></div>
                                                    @endforeach
                                                </div>
                                            @endif
                                            @if($row->rate_number)
                                                ...
                                                <p>
                                                    <a target="_blank" href="{{$service->getDetailUrl()}}">
                                                        <i class="fa fa-long-arrow-right"
                                                           aria-hidden="true"></i> {{ __("View :name",["name"=>$service->getModelName() ])}}
                                                    </a>
                                                </p>
                                            @else
                                                {{__("[Deleted]")}}
