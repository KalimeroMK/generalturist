...
const UNPAID = 'unpaid';
    ...
    const PROCESSING = 'processing'; // New booking, before payment processing
    const CONFIRMED = 'confirmed'; // Require payment
    const COMPLETED = 'completed'; // like offline - payment
        const CANCELLED = 'cancelled'; // after processing -> confirmed (for offline payment)
    const PAID = 'paid'; //
const PARTIAL_PAYMENT = 'partial_payment';
    ...
    {
        $res = [];
        $total_data = parent::selectRaw('sum(`total`) as total_price , sum( `total` - `total_before_fees` + `commission` - `vendor_service_fee_amount` ) AS total_earning ')->whereNotIn('status',
            static::$notAcceptedStatus)->first();
    ...
    public static function getDashboardChartData($from, $to)
    {
        ...
        if (($to - $from) / DAY_IN_SECONDS > 90) {
            ...
            for (
                $i = strtotime(date('Y-m-d', $from)); $i <= strtotime(date('Y-m-d 23:59:59',
                $to)); $i += HOUR_IN_SECONDS
            ) {
                $dataBooking = parent::selectRaw(implode(",", $sql_raw))->whereBetween('created_at', [
                    date('Y-m-d H:i:s', $i),
    ...
    {
        $res = [];
        $total_money = parent::selectRaw('sum( `total_before_fees` - `commission` + `vendor_service_fee_amount` ) AS total_price , sum( CASE WHEN `status` = "completed" THEN `total_before_fees` - `commission` + `vendor_service_fee_amount` ELSE NULL END ) AS total_earning')->whereNotIn('status',
            static::$notAcceptedStatus)->where("vendor_id", $user_id)->first();
        $total_booking = parent::whereNotIn('status', static::$notAcceptedStatus)->where("vendor_id",
            $user_id)->count('id');
        $total_service = 0;
        $services = get_bookable_services();
        if (!empty($services)) {
    ...
    public static function getEarningChartDataForVendor($from, $to, $user_id)
    {
        ...
        if (($to - $from) / DAY_IN_SECONDS > 90) {
            ...
            for ($month = 1; $month <= 12; $month++) {
                $day_last_month = date("t", strtotime($year."-".$month."-01"));
                $data['labels'][] = date("F", strtotime($year."-".$month."-01"));
                $dataBooking = parent::selectRaw(implode(",", $sql_raw))->where("vendor_id",
                    $user_id)->whereBetween('created_at', [
                    $year.'-'.$month.'-01 00:00:00',
                    $year.'-'.$month.'-'.$day_last_month.' 23:59:59'
                ])->whereNotIn('status', static::$notAcceptedStatus);
                $dataBooking = $dataBooking->first();
            ...
            // Report By Day
            for (
                $i = strtotime(date('Y-m-d', $from)); $i <= strtotime(date('Y-m-d 23:59:59', $to)); $i += DAY_IN_SECONDS
            ) {
    ...
    public static function getStatisticChartData(
        $from,
        $to,
        $statuses = false,
        $customer_id = false,
    ...
    public function getCheckoutUrl()
    {
        ...
        return url(($is_api ? 'api/' : '').app_get_locale(false, false,
                "/").config('booking.booking_route_prefix').'/'.$this->code.'/checkout');
    }
    ...
    public function getDetailUrl($full = true)
    {
        $is_api = request()->segment(1) == 'api';
        if (!$full) {
            return ($is_api ? 'api/' : '').app_get_locale(false, false,
                    "/").config('booking.booking_route_prefix').'/'.$this->code;
        }
        if ($is_api) {
    ...
    public function reloadCalculateTotalBooking()
    {
        ...
        if (!empty($list_fees = $this->buyer_fees)) {
            ...
            $total_buyer_fee = $this->service->calculateServiceFees($list_fees, $this->total_before_fees,
                $this->total_guests);
        ...
        if (!empty($list_fees = $this->vendor_service_fee)) {
            $total_service_fee = $this->service->calculateServiceFees($list_fees, $this->total_before_fees,
                $this->total_guests);
