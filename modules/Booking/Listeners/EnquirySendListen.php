    ...
    public function handle(EnquirySendEvent $event)
    {
        if (!empty(setting_item('booking_enquiry_enable_mail_to_vendor'))) {
            $body = $this->replaceContentEmail($event,
                setting_item_with_lang('booking_enquiry_mail_to_vendor_content'));
            Mail::to(User::find($event->enquiry->vendor_id))->send(new EnquirySendEmail($event->enquiry, $body,
                'vendor'));
        }
            ...
            foreach (self::CODE as $item => $value) {
                switch ($value) {
                    ...
                    case "[vendor_link]":
                        ...
                        $text = '<a href="'.route('user.admin.detail',
                                ['id' => $event->enquiry->vendor_id]).'" target="_blank"> '.$user->first_name." ".$user->last_name.' </a>';
                        $content = str_ireplace($value, $text, $content);
                        break;
                    case "[vendor_name]":
                        $user = $event->enquiry->vendor;
